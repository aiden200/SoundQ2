...
from synth_data_gen.utils import get_classes, generate_paths
...
class SegmentObjects:
    def __init__(self, config, device, log):
        ...
        self.working_dir_gd_sam=config["working_dir_gd_sam"]
    ...
    def convert_videos(self, video_path):
        
        finished_files = {}
        with open(self.result_file, mode='r', newline='') as csvfile:
            csv_reader = csv.reader(csvfile)
            for row in csv_reader:
                if len(row) < 2:
                    continue  # Skip rows that don't have at least two items
                filename = row[0]
                success_code = row[1]
                finished_files[filename] = success_code
        
        for video in videos:
            if video not in finished_files:
                #TODO
                self.segment_video()
                pass
            ...
            with open(self.result_file, mode='w', newline='') as csvfile:
                ...
                for row in finished_files:
                    csv_writer.writerow(row)

    def segment_video(self, input_video_path, output_video_path, tracking_results_dir, segment_result_path):
        ...
        source_frames = Path(self.working_dir_gd_sam)
        ...
        frame_names = [
            p for p in os.listdir(self.working_dir_gd_sam)
            if os.path.splitext(p)[-1] in [".jpg", ".jpeg", ".JPG", ".JPEG"]
        ]
        frame_names.sort(key=lambda p: int(os.path.splitext(p)[0]))

        # init video predictor state
        inference_state = video_predictor.init_state(video_path=self.working_dir_gd_sam)
